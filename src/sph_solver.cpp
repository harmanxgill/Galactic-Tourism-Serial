...
void SPHSolver::computeDensity(vector<Particle>& particles) {
...
void SPHSolver::computePressure(vector<Particle>& particles) {
...
void SPHSolver::computeForces(vector<Particle>& particles) {
    #pragma omp parallel for schedule(dynamic)
    for (auto& pi : particles) {
